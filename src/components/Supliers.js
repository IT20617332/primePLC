...
const Supliers = () => {
  ...
  const addOrEdit = obj => {
    if (currentId == '')
      firebaceDb.child('suppliers').push(
        obj,
        err => {
          if (err)
            console.log(err)
      ...
      firebaceDb.child(`suppliers/${currentId}`).set(
        obj,
        err => {
          if (err)
            console.log(err)
  ...
  const onDelete = key => {
    if (window.confirm('Are You Shure To Delete this Supplier ? ')) {
      firebaceDb.child(`suppliers/${key}`).remove(
        err => {
          if (err)
            console.log(err)
          else
  ...
  return (
    <div>
      ...
      <a href="/"><Button variant="outline-dark" >Back</Button></a>
        ...
        <Container>
          <Row>
            <Col><Suplierform {...({ addOrEdit, currentId, suplierObjects })} /></Col>
            <Col>
