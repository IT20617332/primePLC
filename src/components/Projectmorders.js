...
const Projectmorders=()=>{
  ...
  var [currentIds,setCurrentIds]=useState('')
   ...
   const addOrEditz= obj=>{
    ...
    firebaceDb.child(`orders/${currentIds}`).set(
      obj,
      err =>{
          if(err)
          console.log(err)
...
if (window.confirm('Are You Shure To Delete this Order ? ')){
  firebaceDb.child(`orders/${key}`).remove(
    err =>{
        if(err)
        console.log(err)
        else
...
<Container>
  <Row>
    <Col><Orderform {...({addOrEditz,currentIds,orderObjects})}/></Col>
